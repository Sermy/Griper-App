package com.griper.griperapp.cam.internal.utils;
...
public final class CameraHelper {
    ...
    public static File getOutputMediaFile(Context context, @CamConfiguration.MediaAction int mediaAction) {
        ...
        if (mediaAction == CamConfiguration.MEDIA_ACTION_PHOTO) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator +
                    "IMG_" + timeStamp + ".jpg");
        } else if (mediaAction == CamConfiguration.MEDIA_ACTION_VIDEO) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator +
                    "VID_" + timeStamp + ".mp4");
        } else {
            return null;
    ...
    public static Size getPictureSize(List<Size> choices, @CamConfiguration.MediaQuality int mediaQuality) {
        ...
        if (mediaQuality == CamConfiguration.MEDIA_QUALITY_HIGHEST) {
            result = maxPictureSize;
        } else if (mediaQuality == CamConfiguration.MEDIA_QUALITY_LOW) {
        ...
        } else if (mediaQuality == CamConfiguration.MEDIA_QUALITY_HIGH) {
        ...
        } else if (mediaQuality == CamConfiguration.MEDIA_QUALITY_MEDIUM) {
            if (choices.size() == 2) result = minPictureSize;
            else {
                int mediumQualityIndex = choices.size() / 2;
                result = choices.get(mediumQualityIndex);
            }
        } else if (mediaQuality == CamConfiguration.MEDIA_QUALITY_LOWEST) {
    ...
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public static Size getPictureSize(Size[] sizes, @CamConfiguration.MediaQuality int mediaQuality) {
...
//                return size;
    ...
    public static CamcorderProfile getCamcorderProfile(int currentCameraId, long maximumFileSize, int minimumDurationInSeconds) {
        if (maximumFileSize <= 0)
            return CamcorderProfile.get(currentCameraId, CamConfiguration.MEDIA_QUALITY_HIGHEST);

        int[] qualities = new int[]{CamConfiguration.MEDIA_QUALITY_HIGHEST,
                CamConfiguration.MEDIA_QUALITY_HIGH, CamConfiguration.MEDIA_QUALITY_MEDIUM,
                CamConfiguration.MEDIA_QUALITY_LOW, CamConfiguration.MEDIA_QUALITY_LOWEST};
        ...
        return CameraHelper.getCamcorderProfile(CamConfiguration.MEDIA_QUALITY_LOWEST, currentCameraId);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public static CamcorderProfile getCamcorderProfile(@CamConfiguration.MediaQuality int mediaQuality, String cameraId) {
        if (TextUtils.isEmpty(cameraId)) {
            return null;
        }
        int cameraIdInt = Integer.parseInt(cameraId);
    ...
    public static CamcorderProfile getCamcorderProfile(@CamConfiguration.MediaQuality int mediaQuality, int cameraId) {
