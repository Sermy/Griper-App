package com.griper.griperapp.cam.internal.manager.impl;
...
public class Camera1Manager extends BaseCameraManager<Integer, SurfaceHolder.Callback, Camera.Parameters, Camera>
    ...
    @Override
    public Size getPhotoSizeForQuality(@CamConfiguration.MediaQuality int mediaQuality) {
    ...
    protected void prepareCameraOutputs() {
        try {
            if (configurationProvider.getMediaQuality() == CamConfiguration.MEDIA_QUALITY_AUTO) {
    ...
    private void startPreview(SurfaceHolder surfaceHolder) {
        try {
            ...
            else if (configurationProvider.getMediaAction() == CamConfiguration.MEDIA_ACTION_PHOTO)
            ...
            if (Build.VERSION.SDK_INT > 13
                    && (configurationProvider.getMediaAction() == CamConfiguration.MEDIA_ACTION_VIDEO
                    || configurationProvider.getMediaAction() == CamConfiguration.MEDIA_ACTION_UNSPECIFIED)) {
//                parameters.setRecordingHint(true);
    ...
    private void setFlashMode(Camera camera, Camera.Parameters parameters, @CamConfiguration.FlashMode int flashMode) {
        try {
            switch (flashMode) {
    ...
    private void setCameraPhotoQuality(Camera camera) {
        ...
        if (configurationProvider.getMediaQuality() == CamConfiguration.MEDIA_QUALITY_LOW) {
            parameters.setJpegQuality(50);
        } else if (configurationProvider.getMediaQuality() == CamConfiguration.MEDIA_QUALITY_MEDIUM) {
            parameters.setJpegQuality(75);
        } else if (configurationProvider.getMediaQuality() == CamConfiguration.MEDIA_QUALITY_HIGH) {
            parameters.setJpegQuality(100);
        } else if (configurationProvider.getMediaQuality() == CamConfiguration.MEDIA_QUALITY_HIGHEST) {
            parameters.setJpegQuality(100);
        }
        parameters.setPictureSize(photoSize.getWidth(), photoSize.getHeight());
    ...
    @Override
    protected int getPhotoOrientation(@CamConfiguration.SensorPosition int sensorPosition) {
        int rotate;
    ...
    @Override
    protected int getVideoOrientation(@CamConfiguration.SensorPosition int sensorPosition) {
        int degrees = 0;
        switch (sensorPosition) {
            case CamConfiguration.SENSOR_POSITION_UP:
                degrees = 0;
                break; // Natural orientation
            case CamConfiguration.SENSOR_POSITION_LEFT:
                degrees = 90;
                break; // Landscape left
            case CamConfiguration.SENSOR_POSITION_UP_SIDE_DOWN:
                degrees = 180;
                break;// Upside down
            case CamConfiguration.SENSOR_POSITION_RIGHT:
                degrees = 270;
                break;// Landscape right
        }
