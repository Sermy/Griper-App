package com.griper.griperapp.internal.ui;
...
import com.griper.griperapp.internal.ui.view.AspectFrameLayout;
import com.griper.griperapp.internal.utils.Size;
import com.griper.griperapp.internal.utils.Utils;

/**
 * Created by Sarthak on 26-02-2017
 */

abstract public class CamCameraActivity<CameraId> extends Activity
    ...
    protected int sensorPosition = CamConfiguration.SENSOR_POSITION_UNSPECIFIED;
    @CamConfiguration.DeviceDefaultOrientation
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (defaultOrientation == Configuration.ORIENTATION_LANDSCAPE) {
            deviceDefaultOrientation = CamConfiguration.ORIENTATION_LANDSCAPE;
        } else if (defaultOrientation == Configuration.ORIENTATION_PORTRAIT) {
            deviceDefaultOrientation = CamConfiguration.ORIENTATION_PORTRAIT;
        }
            ...
            if (sensorEvent.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
                if (sensorEvent.values[0] < 4 && sensorEvent.values[0] > -4) {
                    if (sensorEvent.values[1] > 0) {
                        // UP
                        sensorPosition = CamConfiguration.SENSOR_POSITION_UP;
                        degrees = deviceDefaultOrientation == CamConfiguration.ORIENTATION_PORTRAIT ? 0 : 90;
                    } else if (sensorEvent.values[1] < 0) {
                        // UP SIDE DOWN
                        sensorPosition = CamConfiguration.SENSOR_POSITION_UP_SIDE_DOWN;
                        degrees = deviceDefaultOrientation == CamConfiguration.ORIENTATION_PORTRAIT ? 180 : 270;
                    }
                } else if (sensorEvent.values[1] < 4 && sensorEvent.values[1] > -4) {
                    if (sensorEvent.values[0] > 0) {
                        // LEFT
                        sensorPosition = CamConfiguration.SENSOR_POSITION_LEFT;
                        degrees = deviceDefaultOrientation == CamConfiguration.ORIENTATION_PORTRAIT ? 90 : 180;
                    } else if (sensorEvent.values[0] < 0) {
                        // RIGHT
                        sensorPosition = CamConfiguration.SENSOR_POSITION_RIGHT;
                        degrees = deviceDefaultOrientation == CamConfiguration.ORIENTATION_PORTRAIT ? 270 : 0;
                    }
                }
